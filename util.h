...
inline bool ConvertToIntW(wchar_t* ps, wchar_t* pe, int64_t& ret_val, INT_VALUE_LITERAL_FORMAT literal_fmt = FMT_AUTO)
{
	ret_val = 0;
	bool bNegative = false;
	...
	if (literal_fmt == FMT_AUTO)
	{
		if (pe > ps + 2 && _wcsnicmp(ps, L"0x", 2) == 0)	// hex value
		{
			ps += 2;
			literal_fmt = FMT_HEX;
		}
		...
		else if (pe > ps + 2 && _wcsnicmp(ps, L"0b", 2) == 0)
		{
			ps += 2;
			literal_fmt = FMT_BIN;
		}
	...
	{
		if (pe > ps + 2 && _wcsnicmp(ps, L"0b", 2) == 0)	// binary value
			ps += 2;
		else if (*(pe - 1) == 'b' || *(pe - 1) == 'B')
			pe--;
	}
...
inline std::string doublecompactstring(double d)
{
	char buf[128] = { 0 };
...
inline std::string floatcompactstring(float d)
{
	char buf[128] = { 0 };
