...
bool ImageProcess::GetImageDrawRect(UINT target_width, UINT target_height, UINT image_width, UINT image_height, D2D1_RECT_F& dst_rect, ImageTransformMode tm_mode)
{
	if (target_width == 0 || target_height == 0 || image_width == 0 || image_height == 0)
		return false;

	if (tm_mode == IMG_TM_PADDING_RESIZE)
	{
...
HRESULT ImageProcess::ToTensor(const TCHAR* cszImageFile, torch::Tensor& tensor)
{
	...
	if (!bDynamic && (outWidth != uiWidth || outHeight != uiHeight))
	{
		...
		if (GetImageDrawRect(outWidth, outHeight, uiWidth, uiHeight, dst_rect, m_image_transform_mode))
